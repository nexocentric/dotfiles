...
if [[ $_ != $0 ]]; then
        ...
        git init
        git remote add https://github.com/nexocentric/dotfiles.git
        git remote -v
        
        if [[ -f ~/.gitignore ]]; then
                mv ~/.gitignore .gitignore-original-dotfiles
        fi
        
        git pull origin master
        
        write_error_message "Clone of dotfiles completed sucessfully!"
fi
